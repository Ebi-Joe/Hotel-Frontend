...
function Home() {
  ...
  const availabilityHandler = async (e) => {
    ...
    setError('');
    setLoading(true); // Start loading for availability check
    try {
          ...
          <div className="rooms md:flex max-w-3xl lg:max-w-5xl m-auto py-24">
            ...
            <div className="mb-8 my-4 bg-black h-fit max-w-lg md:max-w-6xl mx-auto" id="check-availability">
              <div className="mini p-4 md:p-0 md:w-[30vw] lg:w-[20vw]">
                <form onSubmit={availabilityHandler}>
                  {error && <div className="error-message text-red-500 font-semibold bg-red-100 m-4 p-2">{error}</div>}
                  <div className="in m-4">
                    <input onChange={(e) => setStartDate(e.target.value)} type="date" placeholder='CheckIn Date' className='w-full border-[1px] border-gray-600 p-2' min={today} />
                  </div>
                  <div className="in m-4">
                    <input onChange={(e) => setEndDate(e.target.value)} type="date" placeholder='CheckOut Date' className='w-full border-[1px] border-gray-600 p-2' />
                  </div>
                  <div className="m-4">
                    <select onChange={(e) => setTotalRooms(e.target.value)} className='w-full border-[1px] border-gray-600 p-2'>
